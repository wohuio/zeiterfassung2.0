  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.organization_id) {
        router.push(`/crm/organizations/${formData.organization_id}`);
      } else {
        router.push('/crm/persons');
      }
    } catch (err: any) {
      console.error('Error creating person:', err);
      setError(err.message || 'Fehler beim Erstellen der Person');
    ...
    <div className="min-h-screen bg-bg-primary relative overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <motion.div
          ...
          className="mb-8"
        >
          <h1 className="text-title-1 gradient-text mb-2">Neue Kontaktperson</h1>
          <p className="text-body-md text-text-secondary">
            Erstellen Sie einen neuen Ansprechpartner
          </p>
        </motion.div>
        ...
        >
          <div className="space-y-6">
            ...
            <div className="flex gap-4 pt-4">
              ...
              >
                {isSubmitting ? 'Wird erstellt...' : 'Person erstellen'}
              </Button>
              <Button
                type="button"
                onClick={() => router.back()}
